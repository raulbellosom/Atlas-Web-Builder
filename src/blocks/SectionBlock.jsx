...
export const SectionBlock = defineBlock({
  ...
  groups: [
    ...
    { label: 'Estilo', fields: ['background', 'radius', 'shadow', 'border', 'borderColor'] },
  ],
  slots: { children: {} },
  render: ({
    paddingY,
    ...
    const borderTop =
      border === 'top' || border === 'both' || border === 'all'
        ? `1px solid ${borderVal}`
        : undefined
    const borderBottom =
      border === 'bottom' || border === 'both' || border === 'all'
        ? `1px solid ${borderVal}`
        : undefined
    ...
    const sVar = shadow && shadow !== 'none' ? shadowVar(shadow, 'none') : undefined
